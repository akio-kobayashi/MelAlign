...
class Mel2MelDataset(Dataset):
    ...
    def __init__(self, csv_path: str or Path, map_location: str = 'cpu') -> None:
        ...
        # Compute global pitch stats
        all_f0 = []
        for row in self.rows:
            data = torch.load(row['source'], map_location='cpu')
            log_f0 = data['log_f0']
            all_f0.append(log_f0)
        all_f0 = torch.cat(all_f0, dim=0)
        self.mean_f0 = all_f0.mean().item()
        self.std_f0  = all_f0.std().item()
        torch.save({'pitch_mean': self.mean_f0, 'pitch_std': self.std_f0}, 'deaf_stats.pt')
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        src_mel   = src['mel'].float()
        src_pitch = src['log_f0'].float()
        tgt_mel   = tgt['mel'].float()
        tgt_pitch = tgt['log_f0'].float()

        # Normalize pitch
        src_pitch = (src_pitch - self.mean_f0) / (self.std_f0 + 1e-9)
        tgt_pitch = (tgt_pitch - self.mean_f0) / (self.std_f0 + 1e-9)
